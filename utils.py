...
def merge_inputs(queries):
    ...
    returns = {key: default_collate([d[key] for d in queries]) for key in queries[0]
               if key != 'point_gt' and key!= 'pc_rotated'and key!= 'rgb' }
    for input in queries:
        point_clouds.append(input['point_gt'])
        pc_rotated.append(input['pc_rotated'])
        rgb_list.append(input['rgb'])
    returns['point_gt'] = point_clouds
    returns['pc_rotated'] = pc_rotated
    returns['rgb'] = rgb_list
    return returns
